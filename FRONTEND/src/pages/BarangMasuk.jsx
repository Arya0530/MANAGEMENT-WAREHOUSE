...
export default function BarangMasuk() { // Nama file tetep BarangMasuk biar nggak ngerusak App.jsx lu
  // State buat Tab
  const [activeTab, setActiveTab] = useState('masuk'); // Default nampilin tab masuk
    ...
    <div className="min-h-screen bg-gray-light p-8">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
        ...
        <div className="flex border-b border-gray-300">
          ...
          <button 
            className={`flex-1 py-4 text-center font-bold text-lg transition ${activeTab === 'keluar' ? 'bg-white text-orange-600 border-b-4 border-red-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <select className="w-full p-3 border rounded focus:ring-2 focus:ring-navy-main" value={idBarang} onChange={(e) => setIdBarang(e.target.value)} required>
              {listBarang.map((brg) => (
                <option key={brg.id_barang || brg.ID_BARANG} value={brg.id_barang || brg.ID_BARANG}>
                  {brg.nama_barang || brg.NAMA_BARANG} (Stok: {brg.stok || brg.STOK})
                </option>
              ))}
            </select>
          </div>

          {/* Kondisi: Kalau tab Masuk, munculin Supplier */}
          {activeTab === 'masuk' && (
            <div>
          ...
          {/* Kondisi: Kalau tab Keluar, munculin Tujuan */}
          {activeTab === 'keluar' && (
            <div>
              <label className="block text-gray-700 font-medium mb-2">Tujuan Pengiriman</label>
              <textarea className="w-full p-3 border rounded focus:ring-2 focus:ring-red-600" placeholder="Contoh: Dikirim ke Toko Cabang..." value={tujuan} onChange={(e) => setTujuan(e.target.value)} required rows="2"></textarea>
            </div>
          )}

          <div>
            <label className="block text-gray-700 font-medium mb-2">Quantity (Kuantitas)</label>
            <input type="number" min="1" className="w-full p-3 border rounded focus:ring-2 focus:ring-navy-main" value={qty} onChange={(e) => setQty(e.target.value)} required />
          </div>

          <button type="submit" className={`w-full text-white font-bold py-3 rounded-lg shadow-lg transition text-lg mt-4 ${activeTab === 'masuk' ? 'bg-navy-main hover:bg-blue-900' : 'bg-red-600 hover:bg-red-800'}`}>
